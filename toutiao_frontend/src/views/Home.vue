...
// 监听路由变化
watch(
  [() => route.query.categoryId, () => newsStore.categories.length],
  ([newCategoryId]) => {
    if (newCategoryId && newsStore.categories.length) {
      ...
      if (index !== -1) {
        const tabChanged = activeTab.value !== index
        activeTab.value = index

        // 索引未变化时，activeTab 的 watcher 不会触发。
        if (!tabChanged) {
