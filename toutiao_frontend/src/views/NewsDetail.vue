...
const loadNewsDetail = async (id) => {
  ...
  console.log('新闻详情:', newsStore.newsDetail)
  console.log('相关新闻:', newsStore.newsDetail.relatedNews)
...
// 同一个详情组件内切换 /news/detail/:id 时，组件不会重新挂载，需要监听 id 变化。
watch(
  newsId,
  async (id) => {
    await loadNewsDetail(id)
  },
  { immediate: true }
)
</script>
