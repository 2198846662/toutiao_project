...
async def get_news_categories(
    ...
    limit: int = Query(10, description="返回的记录数", ge=1),
    db: AsyncSession = Depends(get_db)
    ):
    #先获取数据库里面的新闻分类数据 -> 先定义模型类  -> 封装查询数据的方法
    categories = await news_cache.get_categories(db, skip, limit)  #调用crud封装好的方法，获取数据
    return {
        "code": 200,
        "msg": "获取新闻分类成功",
        "data": categories
...
async def get_news_list(
    ...
    page_size: int = Query(10, description="每页记录数", alias="pageSize", ge=1, le=100),
    db: AsyncSession = Depends(get_db)
    ):
    # 思路：处理分页规则 -> 查询新闻列表 -> 计算总量 -> 计算是否有更多数据
    skip = (page - 1) * page_size
    limit = page_size
    news_list = await news_cache.get_news_list(db, category_id, skip, limit)  #调用crud封装好的方法，获取数据
    total = await news.get_news_count(db, category_id)  #查询总量
    news_items = [
        NewsItemBase.model_validate(item).model_dump(by_alias=True)
...
async def get_news_detail(
    ...
    #获取新闻详情 + 浏览量+1 + 相关新闻
    news_detail = await news_cache.get_news_detail(db, news_id)  #调用crud封装好的方法，获取数据
    if not news_detail:
    ...
    await news_cache.set_news_detail_cache(news_detail)
    
    related_news = await news_cache.get_related_news(db, news_detail.id, news_detail.category_id)  #查询相关新闻
