...
推荐在 `toutiao_backend/.env` 中配置数据库连接信息，真实账号和密码不要提交到 GitHub：
...
```env
DATABASE_URL=mysql+aiomysql://your-db-user:your-db-password@127.0.0.1:3306/news_app?charset=utf8mb4
```
...
说明：README 中的 `your-*` 都是占位符，需要替换为你本地或服务器上的真实配置。真实密码、内网 IP、API Key 只放在 `.env` 里。
